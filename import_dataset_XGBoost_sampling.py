...
def normalize_action(action):
    ...
    if 'fold' in action:
        ...
        return 'raise'  # or keep as 'allin' if you want a separate class
    elif 'bb' in action:
        return 'raise'
    else:
...
# Step 3: Balance the dataset manually
n_samples = 20000
n_samples_raise = 10000
df_raise = df[df[target] == 'raise'].sample(n=n_samples_raise, replace=True)
df_call  = df[df[target] == 'call'].sample(n=n_samples, replace=True)
df_fold  = df[df[target] == 'fold'].sample(n=n_samples, replace=True)
df_balanced = pd.concat([df_raise, df_call, df_fold])
df = df_balanced

# Step 3.5: Encode the label (target) AFTER resampling
...
X = pd.concat([
    ...
    df[['facing_raise', 'num_raises', 'estimated_pot', 'last_raise_size',
        'num_players_still_in', 'to_call', 'pot_odds', 'is_3bet_plus']]
], axis=1)
y = df['label']
